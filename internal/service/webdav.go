...
import (
	...
	"github.com/milome/code-agent-lens/internal/webdav"
)
...
func (w *WebDAVService) UpdateWebDAVConfig(url, username, password string) error {
	webdavConfig := &config.WebDAVConfig{
		...
		Username:   username,
		Password:   password,
		ConfigPath: "/CodeAgentLens/config",
		StatsPath:  "/CodeAgentLens/stats",
	}
...
func (w *WebDAVService) BackupToWebDAV(filename string) error {
	...
	tempDir := filepath.Join(homeDir, ".CodeAgentLens", "temp")
