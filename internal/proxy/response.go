...
func (p *Proxy) handleNonStreamingResponse(w http.ResponseWriter, resp *http.Response, endpoint config.Endpoint, trans transformer.Transformer, obs *observability.RequestRecorder) (int, int, error) {
	var bodyBytes []byte
	var err error
	...
	resp.Body.Close()
	if obs != nil {
		obs.WithSpan("code-agent-lens.transform.response", func() {})
		obs.WriteHeaders("upstream.response.headers.json", resp.Header)
		obs.WriteBytes("upstream.response.body.raw", bodyBytes)
		if resp.Header.Get("Content-Encoding") == "gzip" {
			obs.WriteJSON("upstream.response.metadata.json", map[string]any{"content_encoding": "gzip", "stored_body": "decompressed"})
		}
	}
	...
	if obs != nil {
		obs.WriteBytes("upstream.response.transformed.raw", transformedResp)
	}
