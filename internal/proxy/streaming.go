...
func (p *Proxy) handleStreamingResponse(w http.ResponseWriter, resp *http.Response, endpoint config.Endpoint, trans transformer.Transformer, transformerName string, thinkingEnabled bool, modelName string, bodyBytes []byte, credentialID int64, obs *observability.RequestRecorder) (int, int, string) {
	streamStart := time.Now()
	if obs != nil {
		defer func() {
			obs.RecordDuration("code-agent-lens_stream_duration_ms", time.Since(streamStart))
		}()
		obs.WriteHeaders("upstream.response.headers.json", resp.Header)
	}
	...
	for scanner.Scan() && !streamDone {
		...
		if strings.Contains(line, "data: [DONE]") {
			...
			if obs != nil {
				obs.AppendStreamEvent("stream.raw.events.jsonl", eventCount+1, eventData)
			}
			...
			if err == nil && len(transformedEvent) > 0 {
				...
				if obs != nil {
					obs.AppendStreamEvent("stream.transformed.events.jsonl", eventCount+1, transformedEvent)
				}
		...
		if line == "" {
			...
			if obs != nil {
				obs.AppendStreamEvent("stream.raw.events.jsonl", eventCount, eventData)
			}
			...
			if err != nil {
				...
				if obs != nil {
					obs.AppendStreamEvent("stream.transformed.events.jsonl", eventCount, transformedEvent)
				}
				...
				if _, writeErr := w.Write(transformedEvent); writeErr != nil {
					...
					if obs != nil {
						obs.SetStatus("client_disconnected")
						obs.WriteJSON("error.json", map[string]any{
