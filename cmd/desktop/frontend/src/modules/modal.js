...
export function closeWelcomeModal() {
    ...
    if (dontShowAgain) {
        localStorage.setItem('CodeAgentLens_welcomeShown', 'true');
    }
...
export async function showChangelogIfNewVersion() {
    ...
    const lastVersion = localStorage.getItem('CodeAgentLens_lastVersion');

    if (lastVersion && lastVersion !== currentVersion) {
        setTimeout(() => showChangelogModal(), 600);
    }
    localStorage.setItem('CodeAgentLens_lastVersion', currentVersion);
}

export function showWelcomeModalIfFirstTime() {
    const hasShown = localStorage.getItem('CodeAgentLens_welcomeShown');
    if (!hasShown) {
        setTimeout(() => {
            showWelcomeModal();
        }, 500);
...
export function openGitHub() {
    if (window.go?.main?.App) {
        window.go.main.App.OpenURL('https://github.com/milome/code-agent-lens');
    }
}
