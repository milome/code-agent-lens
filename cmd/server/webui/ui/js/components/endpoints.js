...
class Endpoints {
    ...
    getTestStatus(endpointName) {
        try {
            const statusMap = JSON.parse(localStorage.getItem('CodeAgentLens_endpointTestStatus') || '{}');
    ...
    saveTestStatus(endpointName, success) {
        try {
            ...
            localStorage.setItem('CodeAgentLens_endpointTestStatus', JSON.stringify(statusMap));
        } catch (error) {
